...
%}
%code requires {
	#define MaxNameOfVariable 20
	#define MaxNameOffunname 20
	#define MaxNameOfparam 20
	...
	variable_pointer createvariable(char name[MaxNameOfVariable]);
...
defstmt		: '(' DEFINE variable exp ')' 	{ 
												...
												/*variable_pointer test = variablehead;
												printf("variable->\n");
												while(test){
													printf("name=%s val=%d\n", test->name, test->val->value_num);
													test = test->next;
												}
												test = functionhead;
												printf("function->\n");
...
funcall		: '(' funexp params ')'			{
			...
			|	'(' ID ')'					{ 
												$$ = findvariable(functionhead, createvariable($2))->val;
											}
			;
params		: params exp					{ 
												i = i+1;
...
variable_pointer findvariable(variable_pointer head, variable_pointer target){
	...
	while((current->next) &&  strcmp(current->name, target->name)!=0)
		current = current->next;
	//printf("current=%s strcmp=%d\n", current->name, strcmp(current->name,id));
